#!/usr/bin/env bash
# cd $CYTOKIT_ANALYSIS_REPO_DIR/analysis/experiments/spheroid/20190703-sta-titration; source env.sh && bash -e pipeline.sh

#EXPERIMENTS=`cat experiments.csv | tail -n +2 | grep 'sta-00.125-20um-s-XY01'`
#EXPERIMENTS=`cat experiments.csv | tail -n +2 | grep 'sta-01.000-20um-s-XY03'`
#EXPERIMENTS=`cat experiments.csv | tail -n +2 | grep 'untreated' | grep 'unstained'`
...
do
    ...
    # * Variants v01 and v02 are comparable to v00 and v01 of co-culture titration experiments w/ custom pipeline
    # (v00 is original config with no spheroid segmentation)
    ...
    do
        ...
        if [ "$VARIANT" == "v00" ]; then
            echo "Clearing CP results at $OUTPUT_DIR/cytometry/cellprofiler/results/"
            [ -e $OUTPUT_DIR/cytometry/cellprofiler/results ] && rm -rf $OUTPUT_DIR/cytometry/cellprofiler/results
