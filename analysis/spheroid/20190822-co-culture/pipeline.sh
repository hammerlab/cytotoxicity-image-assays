#!/usr/bin/env bash
# cd $CYTOKIT_ANALYSIS_REPO_DIR/analysis/experiments/spheroid/20190822-co-culture; source env.sh && bash -e pipeline.sh

#EXPERIMENTS=`cat experiments.csv | tail -n +2 | grep '0250kT-0uM-np-XY07-1'`
#EXPERIMENTS=`cat experiments.csv | tail -n +2 | grep -v '0250kT-0uM-np-XY07-1' | sort -R --random-source=/dev/zero`
